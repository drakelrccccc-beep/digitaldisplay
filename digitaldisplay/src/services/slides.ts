...
export class SlidesService {
  ...
  public static toPresentationUrl(url: string): string {
    try {
      ...
      const match = urlObj.pathname.match(/\/d\/(?:e\/)?([a-zA-Z0-9-_]+)/);
      if (!match || !match[1]) {
      ...
      const presentUrl = urlObj.pathname.includes('/d/e/')
        ? `https://docs.google.com/presentation/d/e/${presentationId}/pub?start=true&loop=true&delayms=3000`
        : `https://docs.google.com/presentation/d/${presentationId}/embed?start=true&loop=true&delayms=3000`;
